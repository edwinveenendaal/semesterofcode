...
function showAdminPage($show_action, $show_last = FALSE) {
    ...
    if (($show_action == 'administer') || ($show_action == 'overview')) {
        ...
        echo renderTabs($nr_tabs, null, 'admin_page-', _PROPOSAL_OBJ, $data, 0, TRUE, renderProposals('draft', null, 'admin_page-1', TRUE), 1, 'proposal');
    }
    $s = '';
    for ($i = 1; $i <= $nr_tabs; $i++) {
        $s .= ($i > 1) ? ', ' : '';
...
function showSupervisorPage($show_action, $show_last = FALSE) {
    ...
    if (!$institutes->rowCount()) {
        ...
        echo tt('Register yourself with your institute at %1$s using the code you got or ask your Semester of Code Program organiser.', '<a href="' . _WEB_URL . "/user/$my_id/edit\">" . t('your account') . '</a>');
    } else {
...
function showInstituteGroupsAdminPage($my_institute, $show_last) {
    ...
    echo renderTabs($nr3, 'Group', $tab_id_prefix, _STUDENT_GROUP, $data3, $id, TRUE, (($nr3 > 1) ?
                    renderOrganisation(_STUDENT_GROUP, $my_group, null, "${tab_id_prefix}$current_tab") :
                    tt('There is no group yet. Click "%1$s" to add one.', t('Add'))), $current_tab);
    ?>
    <script type="text/javascript">
...
// 		 [translate, label, action, type, id, extra GET arguments, render with rich text area, render tab to the right]
    ...
    echo renderTabs($nr2, t('My Group'), 'member_page-', _INSTITUTE_GROUP, $data2, $my_institute->inst_id, TRUE, renderUsers(_INSTADMIN_TYPE, '', $my_institute->inst_id, _INSTITUTE_GROUP, TRUE) .
...
function showInstituteAdminPage($my_institute, $action = 'administer') {
    ...
    if ($action == 'administer') {
        if (vals_soc_access_check('dashboard/institute/administer/add_or_delete')) {
            $data[] = array(1, 'Delete', 'delete', _INSTITUTE_GROUP, $my_institute->inst_id, '', false, 'delete');
            $nr++;
            $tabs[] = "'${tab_id_prefix}$next_tab'";
            $next_tab = 3;
        }
        $data[] = array(1, 'Edit', 'edit', _INSTITUTE_GROUP, $my_institute->inst_id, '', false, 'editing');
        $nr++;
        $tabs[] = "'${tab_id_prefix}$next_tab'";
    }

    //[number of tabs, label start, tab id start, type, data, id, render targets, active target content, active tab]
    echo renderTabs($nr, '', $tab_id_prefix, _INSTITUTE_GROUP, $data, $my_institute->inst_id, TRUE, renderOrganisation(_INSTITUTE_GROUP, $my_institute, null, "${tab_id_prefix}1", false));
    ?>
    <script type="text/javascript">
...
function showOrganisationMembersPage($organisations) {
    ...
    if ($data) {
        ...
        echo renderTabs( --$tab_offset, 'Org', 'mentor_page-', _ORGANISATION_GROUP, $data, null, TRUE, renderUsers(_ORGADMIN_TYPE, '', $first_tab_group_id, _ORGANISATION_GROUP, TRUE) .
